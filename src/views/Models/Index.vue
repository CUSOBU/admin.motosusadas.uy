<template>
  <v-container fluid class="py-0">
    <Heading>
      <template #title>
        <h2>{{ $t("models") }}</h2>
        <p>{{ $t("models-management") }}</p>
      </template>
      <template #actions>
        <v-btn color="primary" variant="flat" elevation="0" @click.prevent="router.push({ name: 'models-create' })">
          <v-icon>mdi-plus</v-icon>
          <span class="d-none d-sm-inline">{{ $t("new-model") }}</span>
        </v-btn>
      </template>
    </Heading>
    <SplitView class="mt-3">
      ...
      <models-table :search="filters.keyword" />
    </SplitView>
  </v-container>
</template>
...
import ModelsTable from "./Table.vue";
