<template>
  <confirm-dialog :model-value="show" @update:model-value="$emit('update:show', $event)" @confirmed="remove">
    ...
    {{ $t("delete-model-confirmation", { name: item ? item.name : '-' }) }}
  </confirm-dialog>
</template>
...
export default {
  ...
  setup(props: any, { emit }: any) {
    ...
    const remove = async () => {
      try {
        await store.dispatch('models/deleteModel', props.item.id);
        await store.dispatch('notificator/success', 'model-deleted-success');
        emit('removed');
